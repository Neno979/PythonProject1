...
with open("Score_sheet.json" , "r") as score_file:
    ...
    for score_dict in score_data:
        print("Name: " + score_dict["name"] + ", attempts: " + str(score_dict["attempts"]) + ", time: " + str(score_dict["time"]) + ", guessed number " + str(score_dict["guessed_number"]))
name = input("please enter your name: ")
while True:
    guess = int(input("please guess integer number between 1 and 30: "))
    attempts += 1
    if guess == secret:
        score_data.append({"name": name, "attempts": attempts, "time": str(datetime.datetime.now()), "guessed_number": str(guess) })
